...
API router: deadline detection endpoint.
"""
...
from app.models.deadline import (
    DeadlineExtractionErrorResponse,
...
def get_deadline_service() -> DeadlineService:
    """Return the shared DeadlineService instance."""
...
@router.post(
    ...
    summary="Detect deadlines for extracted actions",
    description=(
        "Takes an ingested `CommunicationRecord` (identified by its `communication_id`), "
        "obtains upstream context (Module 2 understanding, Module 3 actions, and Module 4 "
        "responsibilities if not provided), and determines the deadline for each action.\n\n"
        "Strict boundary: Does NOT extract decisions, approvals, priorities, or memory."
    ),
    responses={
        400: {"model": DeadlineExtractionErrorResponse, "description": "Invalid input or empty content"},
        404: {"model": DeadlineExtractionErrorResponse, "description": "Communication not found"},
        422: {"description": "Validation error in request body"},
        502: {"model": DeadlineExtractionErrorResponse, "description": "LLM failure or schema boundary violation"},
    },
)
async def extract_deadlines(
    ...
    deadline_svc = get_deadline_service()
    ...
    # Step 4: Obtain Module 4 responsibilities if context is requested
    ...
    if request.include_context:
        if request.responsibilities is not None:
            ...
            try:
                resp_result = resp_svc.detect_responsibilities(
                    record=record,
                    actions=actions,
                ...
                logger.error(
                    "Module 4 responsibility detection failed for communication %s: %s",
                    request.communication_id,
                    exc,
                )
                raise HTTPException(
    ...
    # Step 5: Perform Deadline Detection (Module 5)
    try:
        result = deadline_svc.detect_deadlines(
            record=record,
            actions=actions,
            understanding=understanding,
        ...
        logger.error("LLM deadline detection validation failed: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
                "The LLM returned a deadline detection response that violated schema boundaries "
                f"or failed validation: {exc}"
            ),
        ) from exc
    except DeadlineProviderError as exc:
        logger.error("LLM provider error during deadline detection: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="The LLM provider is currently unavailable. Please try again later.",
        ) from exc
    except Exception as exc:
        logger.exception("Unexpected error during deadline detection")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred during deadline detection.",
        ) from exc

    return DeadlineExtractionResponse(data=result)
